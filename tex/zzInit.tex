...
\ifthenelse{\boolean{TOHTML}}{%
  \usepackage{endnotes}                 % tex4ht-compatible endnotes for EPUB
  \newcommand{\printendnotes}{\theendnotes}%
  \newcommand{\SetupBookEndnotes}{%
    \AtBeginDocument{\let\footnote=\endnote}%
  }%
}{%
  \usepackage{enotez}                   % Modern endnotes for print/web
  \ifthenelse{\boolean{TOHTML}}{%
    \setenotez{backref=false,list-name={Notes by Chapter}}%
  }{%
    \setenotez{backref=true,list-name={Notes by Chapter}}%
  }%
  \AtBeginDocument{%
...
% Engine-specific font setup (HTML/EPUB uses pdfLaTeX-compatible fonts)
\ifluatex
  \ifthenelse{\boolean{USELUATEX}\AND\not\boolean{TOHTML}}{%
    \usepackage{fontspec}
...
\else
  \ifthenelse{\boolean{USELUATEX}}{%
    \PackageWarning{zzInit}{USELUATEX true but not LuaLaTeX; using pdfLaTeX fonts}
  }{}
  \usepackage[T1]{fontenc}
  \usepackage{textcomp}
  \usepackage{lmodern}
...
% Math and Software (skip amsmath for tex4ht — LaTeX 2024 kernel defines \iint)
\ifthenelse{\boolean{TOHTML}}{%
  \RequirePackage{amstext}% minimal AMS text support for tex4ht
}{%
  \usepackage{mathtools}
}
\usepackage{wasysym}              % Additional symbols (used in preface.tex)
\ifluatex
  \ifthenelse{\boolean{USELUATEX}}{}{%
    \usepackage{latexsym}
  }{}
...
% biblatex + biber (after amsmath; replaces BibTeX + authorindex)
\ifthenelse{\boolean{TOHTML}}{%
  \usepackage[backend=biber,style=ext-numeric-comp,backref=false,url=true,doi=true,isbn=true,hyperref=auto]{biblatex}
}{%
  \usepackage[backend=biber,style=ext-numeric-comp,backref=true,url=true,doi=true,isbn=true,hyperref=auto]{biblatex}
}
\addbibresource{tex/bib/book.bib}

% Author-only bibliography environment for "Authors Cited"
\defbibenvironment{authorname}{%
  \list{}{%
...
\newcommand{\EmitPrintAuthorIndex}{%
  \printbibliography[env=authorname,heading=none,title={Authors Cited}]%
}

\ifthenelse{\boolean{USESIUNITX}}{%
  \usepackage{siunitx}
}{}

\usepackage{tcolorbox}
\tcbset{colback=gray!5,colframe=gray!50,boxrule=0.5pt,arc=2pt}
\newtcolorbox{sidebar}{colback=blue!5!white,colframe=blue!50!black,title=Sidebar}
...
% microtype, bookmark, cleveref, hyperxmp (after hyperref)
\ifthenelse{\boolean{TOHTML}}{}{%
...
% Summary TOC macro (KOMA/tocbasic style)
\newcommand{\summaryTOC}{%
